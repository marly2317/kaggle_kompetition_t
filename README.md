...
| Эксперимент | Модель | CV accuracy | CV ROC AUC | CV F1 | Kaggle public |
|-------------|--------|-------------|------------|-------|---------------|
| `000_baseline_logreg` | Логистическая регрессия — бейзлайн | 0.798 ± 0.014 | 0.859 | 0.727 | 0.778 |
| `001_best_solution` | **CatBoost — рабочее решение** | 0.835 ± 0.009 | 0.884 | 0.774 | **0.821** |
| `032_catboost_no_drift_features` | CatBoost без дрейфящих фич | 0.829 ± 0.006 | 0.872 | 0.766 | 0.775 |
| `033_lightgbm` | LightGBM — нетюненный | 0.852 ± 0.018 | 0.885 | 0.798 | 0.778 |
| `034_lightgbm_regularized` | LightGBM — регуляризованный | 0.845 ± 0.015 | 0.886 | 0.787 | 0.794 |
| `035_mlp` | MLP (PyTorch, entity embeddings) | 0.815 ± 0.037 | 0.885 | 0.760 | 0.785 |
| `036_mlp_tuned` | MLP — гиперпараметры подобраны Optuna | 0.836 ± 0.016 | 0.886 | 0.777 | 0.797 |
| `037_mlp_tuned_adamw` | MLP — AdamW, перетюнен Optuna | 0.829 ± 0.013 | 0.887 | 0.773 | 0.778 |

CV-метрики — среднее по 5 фолдам; accuracy и F1 при пороге `0.5`, ROC AUC от порога не зависит.
