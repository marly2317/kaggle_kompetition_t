...
def run(config, project_root):
    ...
    _save_importance([r["importance"] for r in fold_results], report_dir)
    save_json(
...
def fit_fold_model(config,x_train,y_train,x_valid,y_valid,categorical,model_params_override=None,fit_params_override=None):
    
        model = get_model(config, params_override=model_params_override)

        model.fit(x_train,y_train,cat_features=categorical,eval_set=(x_valid, y_valid),**get_fit_params(config, fit_params_override))
